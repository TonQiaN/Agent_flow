...
当前问题调查先按症状查 Blackbox Agent Flow 的对应实现、测试与修复历史，核对是否已有解决方案，再进行本项目的复现和修复。记录参考版本及环境差异；借鉴已有修复后仍运行本项目的回归测试。
...
然后按 [文档维护指南](documentation.md) 先写相应决定和开发约定，再开展实现和测试。决定可以与代码、测试、指南一起提交。当前基础检查命令为 `npm run check`，包含依赖边界、构建、测试类型检查及 Node 测试；先运行普通测试文件（并发数为宿主可用并行度与 4 的较小值），再串行运行 src/tests/e2e 的测试文件；文件内部的并发场景保持原样；更完整的集成验证按改动和验收范围确定，实际结果留在 PR 或需要长期保存的 [验证记录](../validation/README.md)。
