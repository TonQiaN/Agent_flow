...
然后按 [文档维护指南](documentation.md) 先写相应决定和开发约定，再开展实现和测试。决定可以与代码、测试、指南一起提交。当前基础检查命令为 `npm run check`，包含依赖边界、构建、测试类型检查及 Node 测试；测试文件并发数为宿主可用并行度与 4 的较小值，文件内部的并发场景保持原样；更完整的集成验证按改动和验收范围确定，实际结果留在 PR 或需要长期保存的 [验证记录](../validation/README.md)。
