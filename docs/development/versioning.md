...
主负责开发者先在相关 Issue 中明确目标与范围，再更新 roadmap 总表。初期只使用计划中、进行中、已发布三种版本状态，状态集中放在总表；它们不替代 Issue 就绪确认或决策生命周期。
...
1. **准备发布记录。** 核对版本整体验收和相关 Issue，通过 PR 将本次 Unreleased 条目移到 `X.Y.Z — 待发布` 标题下，保留 Unreleased；roadmap 保持进行中并关联版本计划。同步必要使用说明和验证证据，未交付事项不算入本版，其他尚未发布的实际变化留在 Unreleased。
