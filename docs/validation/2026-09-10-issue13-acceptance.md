...
切片212e873同步 Harness PR #27 的8fb9dfe，生产源码一致。完整矩阵启用 Docker、CONNECT 及三种已核对 CLI 镜像：普通组357通过、0跳过（13.05秒）；端到端组198项中196通过、2失败、0跳过（694.39秒）。两个失败记录来自同一个登录 cleanup-pending 子用例及其父测试，不能把该命令记为完整通过。

按 Blackbox 优先排查要求复核既有 Runner 清理失败及剩余资源记录。实际差异是本层为代理和任务统一增加完整 Attempt 标记，旧登录测试只按 Attempt 查询却仍假定仅有一个容器，出现2≠1。修正测试为严格验证唯一资源及对应任务/代理两个容器，测试收尾也按唯一资源清理；生产清理与凭据保留条件未变。全部15项真实 PTY/Docker 登录检查随后通过，0跳过（33.43秒），测试类型检查同步复验。其余已通过端到端场景的源码未改，不重复完整11分钟矩阵。

本轮真实进程中断包含订阅 Agent、文件 Gate、转换与 Effect 的完整合成批卷恢复，以及旧资源查询/停止故障、两个恢复进程竞争和连续中断。真实官方模型、机器断电及学生报告仍按各自证据范围单独记录。PR 维持 Draft，等待下层交付、完整提交审阅及远端 CI。

### CI 基础镜像准备

首次Ubuntu Node24/26 CI在联网资源定义、Tutor Script和版本资源探针中出现DOCKER_COMMAND_FAILED；普通测试通过。按Blackbox优先流程检查其Docker夹具与当前测试依赖后，核对本仓库CI只预拉取Alpine，而上述路径还显式解析node:22-bookworm-slim；本机已有该镜像，所以本地完整矩阵未暴露这一准备缺口。现将Node基础镜像加入CI的显式准备步骤，不改产品错误或跳过测试。修正后远端结果继续按实际head核对，不能仅据配置修正声明CI通过。
