...
### 后续取消测试的同类前提（悉尼时间2026-09-16）

head26c3eaf的六个CI检查五个通过，另一个Node24在正常取消测试约393毫秒时触发QUEUE_CLAIM_LOST。先复核同一Blackbox scheduler的正常停止与专门过期边界，再在本项目业务进入后同步停顿350毫秒、随后取消：旧300毫秒测试租约下确定性失败。取消探针保留等待中的业务释放和结果收尾，避免反例遗留任务。

逐项检查本文件其余Worker构造器，普通启动失败、未确认后继、drain、取消、重复接纳、绑定不匹配和未预占来源拒绝均不需要租约到期。这七处也改用默认30秒租约；显式queue.claim、失联进程、旧写入者和专门过期测试的短租约保持。实际生产源码不变，未把失去领取后的取消改成成功。保留两个CI失败日志与两项确定性反例，最终检查按新head重新执行。

第二次修正后，同文件19项通过；完整边界/构建/类型检查、415项普通测试及17项默认E2E再次通过，188项仍按环境跳过。此后只提交测试与文档，不改变已验收的产品源码。
