...
完整 `npm run check` 启用 Docker、受控联网及既有 Codex 0.153.4 镜像：**84 通过、0 失败、0 跳过**。后续仅将 FileManifest/ArtifactStore 类型移到 engine 的可替换接口边界，再次通过构建、测试类型检查和依赖检查；没有改变运行行为。默认 CI 未提供专用镜像及外网环境时仍明确跳过原有 5 项环境测试。
