...
当前实现包含 domain、engine 和 CLI。domain 保存业务类型与身份；engine 的 contracts 注册 JSON Schema，components 分别注册定义、函数实现并协调一次执行；CLI 组合这些公共接口。
...
Run/NodeTask/Attempt 当前只有身份与校验，尚无分配器、调度或持久化。Component 不持有路由、文件系统、Docker、认证或 Harness。可信函数运行在调用进程中，内存副本隔离不等于安全沙箱。
