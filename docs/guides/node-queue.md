...
- 索引最多保存 2,048 个历史 NodeTask，包含已完成项；单任务最多保存 64 个未收束的领取令牌，超过时阻塞。繁忙等待完成封存后清空该批令牌。达到上限明确拒绝，不自动删除历史。配置变化与已存配置不符时拒绝打开；当前索引 schema 为 v2；没有从本地开发 v1 的迁移、压缩或热更新工具。凭据源保留已封存令牌的本地收据，首期没有自动清理。
