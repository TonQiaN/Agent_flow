...
本地队列已接通持久 Run、单节点执行和共同恢复接口。当前提供程序 API；#14 的认证源占用联合验收与 PR 交付尚未完成，不能据此宣称整个 Issue 已交付。取舍见 [P-20260910-node-queue](../../.agents/decisions/product/README.md#p-20260910-node-queue)。
...
const profile = { credentialRef: 'writer', service: 'example', method: 'api-key' };
const queue = new PersistentNodeQueue(store, {
  roles: { producer: 2, fixer: 1 },
  credentials: [{ identity: profile, capacity: 1 }],
  workflows: {
    grading: {
      draft: { role: 'producer', capability: 'agent', credential: profile },
      repair: { role: 'fixer', capability: 'agent', credential: profile },
    },
  },
});
...
认证按 `credentialRef/service/method` 身份计数，别名共用一个容量条目，重复身份配置拒绝。`capacity: null` 表示没有额外的本地认证并发上限，角色上限仍生效；该值不证明远端额度。当前配置是调用方显式提供的调度要求，必须与应用选定的实际 Profile 和 Harness 一致。队列不会改选账号、endpoint 或 Harness。

## Worker 与生命周期

```ts
const worker = new NodeWorker(queue, {
  async open(runId, records) {
    // 应用根据已存 Run 安装当前定义与存储绑定；这里不执行流程。
    // 返回 { compiled, runtime, dispose? }；资源实现留在应用组合层。
    return openApplication(runId, records);
  },
...
`openApplication` 是应用自己的工厂，不是库中预设函数。它应安装与保存定义一致的当前代码、文件存储、Runner 和授权；可选 `dispose(snapshot)` 释放本次组合持有的资源。不要在工厂中执行另一个 Workflow 或跳过正常验收。
...
任务可因 `ROLE_CAPACITY`、`CREDENTIAL_CAPACITY` 或 `CAPABILITY_UNAVAILABLE` 等待。按就绪次序扫描并跳过不能领取的任务；等待没有 Attempt，不消耗业务重试次数。
...
工厂在创建 Attempt 之前失败时，任务 blocked、容量释放；修复配置后可显式重新调度。出现未确认执行时保留容量。前一个节点的成功结果不能作为后一个节点停止的证明。
...
- 索引最多保存 2,048 个历史 NodeTask，包含已完成项；达到上限明确拒绝，不自动删除历史。配置变化与已存配置不符时拒绝打开；当前没有迁移、压缩或热更新工具。
- 队列简单扫描、统一索引写入，接受单机规模限制；没有公平性保证、多主机调度、抢占、优先级或驻留 Agent 池。
- 队列的认证容量与凭据源的登录、刷新和执行互斥是不同事实。现有源互斥仍防止不安全并发，但源在领取前或领取后被其他操作占用时，尚未接通“不生成失败 Attempt、留队等待”的完整联合机制。实际 Profile 与调度配置的一致性也尚未形成自动验证；这是 #14 的明确剩余验收项。
