...
`WorkflowRuntime.startPersisted(compiled, runId, input, store)` 在现有串行执行路径上保存正常运行事实。当前支持 Docker Script、具备相应资源能力的 API key/订阅 Agent、显式确定性 JSON 函数、指定 JSON 文件投影和固定 apply Effect。它提供检查点写入和异步取消确认；各绑定的条件及节点边界恢复见[恢复接口](workflow-recovery.md)。
...
真实 SIGKILL 已验证删除原输入/临时目录后 A 不重跑、旧 B 清理后第 2/3 次 Attempt 完成。确定性函数、固定 Effect、JSON 投影和订阅资源也已完成各自合成中断验证，见[#13 验收对照](../validation/2026-09-10-issue13-acceptance.md)。任意宿主回调、未知操作及临时目录通用崩溃 GC 仍不支持。
