...
当前保存的是正常执行的已结束步骤和当前活动身份；[严格加载与文件引用恢复](workflow-checkpoint-loading.md)已接入；中断后追加旧 Attempt 结果、同 NodeTask 的新 Attempt、跨进程恢复竞争以及 Runner 资源身份持久化/query/stop 均继续实现。没有将任意保存 JSON 转成执行器或可信接纳接口，也没有提供宽松的文件收据导入方法。
