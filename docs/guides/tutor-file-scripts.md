...
本切片提供可持久运行的文件节点绑定，尚未改造 createGradingApplication 的整条运行入口。原始来源事实的应用级创建/重开、文件到 JSON 收据、发布和真实 Agent 的联合持久验收仍需接线。示例中的罐装初批/返修 Script 只用于测试，不冒充模型调用。见[验证](../validation/2026-09-10-tutor-file-scripts.md)。
