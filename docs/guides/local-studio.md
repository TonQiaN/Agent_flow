...
3. 画布上的序号和箭头表示步骤顺序，虚线表示有次数限制的返工路线。可以平移、缩放和拖动节点，拖动只调整这台浏览器的布局；“自动布局”恢复默认位置。默认不显示空详情栏，点击节点或连线才展开详情，关闭后恢复完整画布。窄屏先显示可读的开始节点，可平移浏览或点击“适应画布”看全图。
...
7. “本次设置”显示当时保存的容器镜像摘要、CPU、内存、网络和挂载。不是容器节点或旧记录缺少配置时会说明；不会拿今天的配置替换历史配置。
...
`studio:test` 在 `127.0.0.1:3597` 启动合成测试服务，需要已构建材料镜像；不使用真实 key。覆盖目录与示例边界、完整运行入口、默认节点可读且不重叠、按节点轮次进入子任务、回放定位，以及完整上传、PDF 实际渲染、历史回放、丢失归档、390px 窄屏、返工、重试、失败和取消。CI 在 Linux Node 24/26 运行原 Docker 矩阵和材料/招聘测试，另有 Node 24 Chromium 用户旅程；成功和失败均保留目录/画布截图与浏览器报告，失败另存 trace 和进程日志。
