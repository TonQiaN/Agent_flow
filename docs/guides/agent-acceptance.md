...
`AgentExecutor` 位于 engine，协调一个 Attempt 的文件输入、受信执行驱动、结果接纳和内存收据登记。它不分配业务节点、不选择后续路由，也不代替 Workflow 的 Component 定义与编译。当前切片用 JSON 文件携带业务结构化内容，尚无通用内存 JSON Agent 输出协议。
