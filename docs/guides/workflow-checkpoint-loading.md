...
加载可以检查 running、cancelled 或 failed 记录，但不会改变它们的状态。宿主被终止不代表其容器已停止；后续恢复协调仍须取得 CAS 所有权、核对并停止旧 Runner 资源，再以同一 NodeTask 的新 Attempt 进入正常执行。[恢复认领与旧资源清理](workflow-recovery.md)已接入，新的 Attempt 执行入口尚未开放。
