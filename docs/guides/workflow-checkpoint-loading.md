...
指定 JSON 文件读取器的已接纳值使用 JSON v2 封套携带来源证明，严格加载器要求其配套恢复端口并拒绝降级；一次性请求公开冻结的 contract 用于分派，仍不能由普通 JSON 冒充。加载后恢复转换收据，失败或 dispose 撤销，见[转换指南](json-file-projection.md)。
