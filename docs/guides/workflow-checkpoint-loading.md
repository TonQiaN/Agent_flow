...
加载先严格核对版本、字段、Run 和步骤身份、已接纳输出、当前值及状态，并复用正常执行的路由计算核对历史计数与位置。文件记录再核对归档摘要、contract、收据身份、实际镜像及前序清单；文件内容经过摘要复核和 contract 校验后复制到新的临时 ArtifactStore。旧 fileRef 和清单 ID 保留逻辑关联，实际临时存储 ID 单独管理。复制出的文件可修改，后续物化和原归档保持不变。
