...
新增 Workflow 恢复 CAS 认领与旧资源共同清理，保留原业务快照；并发恢复及旧宿主迟到写入不能覆盖新 revision，查询/停止/释放不明可重试。只读加载同时支持恢复记录，不产生新认领。新 Attempt 执行仍未开放。

Workflow 检查点为 v3：正常 Attempt 保存身份、Runner 资源、结果步骤关联及启动操作进度；资源保存和每次准备/创建/启动前等待 CAS，启动完成必须由实际观测确认，失败阻断后续操作。每次调用的端口拒绝跨身份、重复、重叠和迟到保存；严格加载器拒绝旧未发布 v1/v2 试验记录，不自动补造证据。恢复认领见上，新 Attempt 调度仍未开放。

新增 Runner 实际资源保存和重启后的查询/停止/移除端口：保存失败阻止创建启动，完整身份或环境不匹配拒绝，停止不明保留资源；尚未接入 Workflow 新 Attempt 恢复。

新增严格检查点加载和独立文件引用恢复，核对实际定义、历史、归档和收据并提供失败回滚；修复归档完成时取消可能写入值/步骤不一致记录的时序，尚未开放重启执行。

新增正常 Workflow 检查点写入：共享执行和路由、初始/接纳文件归档、状态 CAS 与持久取消确认；写入失败阻止后继，尚未开放重启执行。

新增断网脚本执行绑定快照与 Docker 镜像冻结，执行消费同一镜像 ID；尚未接入 Run 恢复。

新增实际 Workflow 结构与契约快照，支持跨 Catalog 同名定义一致性核对；尚未包含执行环境或 Run 恢复。
