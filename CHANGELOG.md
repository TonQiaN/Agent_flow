...
Workflow 检查点为 v4：正常 Attempt 保存身份、Runner 资源、结果步骤关联及启动操作进度；资源保存和每次准备/创建/启动前等待 CAS，启动完成必须由实际观测确认，失败阻断后续操作。每次调用的端口拒绝跨身份、重复、重叠和迟到保存；严格加载器拒绝旧未发布 v1/v2/v3 试验记录，不自动补造证据。恢复认领见上，新 Attempt 恢复见上。
