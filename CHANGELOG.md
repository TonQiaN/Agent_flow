...
- 修复 Map/Fork 将显式无效 retry 值静默当作缺省的问题；登记统一复用既有重试校验，失败时不保留部分子计划。

- 修复并行组内已恢复清理、等待凭据的子项导致整组无法取消；串行与整组取消共用清理判据，并保留重新领取时的协作取消与事务保护。

- 修复Worker交还后的迟到心跳取消竞争、重试取消的等待状态残留，以及认证副本尚未创建和失败下游清理时的资源释放判断。
- 修复 Worker 恢复并确认清理后、因凭据繁忙重新排队的任务无法取消；保留旧 Attempt，并通过同一条件事务防止与重新领取冲突。
- 移除未发布且无当前调用的旧 API key 文件快照入口；订阅继续使用独占文件绑定，DeepSeek 继续使用无任务密钥文件的环境绑定。同步 Runner 记录预算及认证/工具指南的当前能力。

- 修复 DeepSeek 文件请求取消后后代仍持有管道、导致队列无法结束的问题：停止本次专属进程组，并等待管道关闭后再处理下一请求。
...
- 新增单层 JSON Map 与固定 Fork：稳定子项身份、确定性汇合、wait-all 和整体 contract；子项共用角色/认证容量与持久重试，已接纳项恢复不重跑。首版单 Component、最多62项，无嵌套、文件或Effect并行；队列索引v3不迁移开发格式。
- 新增扫描件批卷与报告消费端：独立Marker/Reviewer、可信Gate、用户返修、报告及A3 PDF；评分和渲染留在Tutor适配，需要显式安装路径。已验收16张真实作答及18页PDF，采用草稿修正和报告单独续跑。
- Codex支持显式多图片输入及受信宿主配置的多页字节预算；用户确认缺失作答后，仅报告将缺失项计零，原批改不变。
...
- 新增独立 RunRecordStore 与私有 SQLite 状态保存、版本 CAS、损坏拒绝；文件归档先耐久提交，再接纳结果与推进后继，临时句柄释放不删除归档。

- Workflow 保存实际结构、契约、执行定义、固定镜像及非秘密 Profile；严格加载检查点、收据和归档，拒绝漂移及不支持的旧试验格式，不补造执行证据。

- 正常执行与恢复共用有序资源阶段和 CAS：准备、创建、启动前记录意图，启动完成由实际观测确认。恢复先认领当前 revision、核对并清理旧资源，再为同一 NodeTask 创建新 Attempt；已接纳节点不重跑，旧宿主迟到写入不能覆盖新状态。

- Runner 可恢复实际任务、版本探针及 CONNECT 资源；未知启动/停止或清理结果阻止重跑。API key 快照和订阅占用分别恢复，确认终止后才处理订阅刷新，禁止旧占用回滚新配置。
- 配合 FileCredentialStore 的普通订阅执行也在 prepare 内取得资源占用并保存认证收尾结果，与持久执行共用实现；Workflow Run/阶段检查点仍须显式启用，认证记录本身不授予恢复权。

- 固定操作的 apply Effect 可使用持久日志与严格回执恢复：占位耐久后调用服务，已确认结果复用仍需当前授权，pending/unknown 阻止重发。支持版本化 JSON 函数、固定脚本与内置指定 JSON 文件转换的恢复；不将任意宿主闭包视为可恢复实现。

- 新增显式持久批卷应用：从 Run 首个归档恢复原始来源，使用同一评分源码生成 intake/Gate 脚本，连接 Agent、用户返修、JSON 转换来源证明及当前发布授权；删除原宿主来源后仍可恢复，已确认发布不重复执行。使用合成答卷和协议替身验证，真实模型恢复不在本次证据内。

- 文件快照支持同存储借用及目标存储直接物化捕获，减少重复复制；版本探针和 Agent 输入在已登记 Runner 工作区内创建，失败清理共用资源边界。Codex/Claude/DeepSeek Driver 的 inputRoot 选项移除，保留 timeoutMs，详见当前使用指南。
