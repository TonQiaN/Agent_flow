# 开发流程决策索引

以下 ID 是稳定引用入口；正文位置表达生命周期。索引不单独维护状态或结论。

## D-20260909-source-layout

[根 src 下区分应用、业务定义、引擎与具体适配](proposed/D-20260909-source-layout.md)

## D-20260907-documentation-layers

[决策、当前说明与宽松原始资料分层](implemented/D-20260907-documentation-layers.md)

## D-20260907-decision-lifecycle

[用目录管理决策生命周期](implemented/D-20260907-decision-lifecycle.md)

## D-20260907-decision-record-content

[一份自足的决定保存选择、比较与否决理由](implemented/D-20260907-decision-record-content.md)

## D-20260907-mandatory-source-traceability

[强制正式决策绑定原始资料规范与路径](rejected/D-20260907-mandatory-source-traceability.md)

## D-20260907-agents-writing

[决策目录 AGENTS.md 的职责与唯一书写归属](implemented/D-20260907-agents-writing.md)

## D-20260910-root-agents-writing

[根目录 AGENTS.md 作为简短的项目工作入口](implemented/D-20260910-root-agents-writing.md)

## D-20260911-materials-management

[materials 保留本地原始资料，选定内容通过飞书共享](implemented/D-20260911-materials-management.md)

## D-20260907-development-workflow

[以 Issue 为起点的开发与 PR 闭环](proposed/D-20260907-development-workflow.md)

## D-20260908-version-management

[用轻量 Roadmap、变更记录和 Tag 管理版本](implemented/D-20260908-version-management.md)

## D-20260908-issue-classification

[用所属领域与五类交付目标区分主 Issue](implemented/D-20260908-issue-classification.md)

## D-20260908-issue-template-common

[主 Issue 的共通字段与 Sub-issue 的自主表达](implemented/D-20260908-issue-template-common.md)

## D-20260908-issue-template-feature

[功能主 Issue 说明当前场景与期望能力](implemented/D-20260908-issue-template-feature.md)

## D-20260908-issue-template-bug

[缺陷主 Issue 保留行为差异、复现与环境](implemented/D-20260908-issue-template-bug.md)

## D-20260908-issue-template-research

[研究主 Issue 说明未知问题与期望结论](implemented/D-20260908-issue-template-research.md)

## D-20260908-issue-template-decision

[决策主 Issue 说明当前情况与需要作出的选择](implemented/D-20260908-issue-template-decision.md)

## D-20260908-issue-template-maintenance

[维护主 Issue 说明当前问题与期望改进](implemented/D-20260908-issue-template-maintenance.md)
