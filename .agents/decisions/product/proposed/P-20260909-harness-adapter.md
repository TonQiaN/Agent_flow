...
容器启动程序固定实际 dsh 路径、版本、headless profile、只读配置路径及工作目录；仅接收原样任务说明，不接受用户提供的程序、额外 patch、环境或工作路径。启动前读取固定私有 API key 交接文件，核对无链接、仅属主权限、有界 JSON 和明确 schema，仅将 api_key 值放入 dsh 子进程的 DEEPSEEK_API_KEY。该文件是未来宿主凭据组合的内部交接，不改变用户输入/输出合同；不把真实密钥放进 argv 或只读任务配置。固定非秘密开关由程序设置，代理环境只接收 Runner 已有受控代理字段，不继承其他环境。
