...
第一步独立落实文件服务及真实原生工具验证，Bash 沙箱与其他原生进程路径必须随后采用同等约束，未全部完成时不装配为可执行 Harness。文件服务持有独立临时目录并在服务关闭时回收；一次性文件子进程通过有界消息交换结果并在结束后返回；中断会终止并等待子进程关闭，未收尾不继续文件请求。对原生大文件流暂设明确的完整缓冲上限，超过上限失败而不截断成功；后续有实际需求再实现流式传输。任何后端不可用或协议异常都失败，不回退到父进程 I/O。
