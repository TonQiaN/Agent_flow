...
2026-09-11 退出竞争修正：Docker inspect 的状态采样与 attach 进程结束不是同一时刻。若 attach 已结束而刚取得的样本仍是 created/running，后端只重新核对一次容器状态；新样本为 exited 才返回退出，仍运行则按传输提前结束失败，查询未知不猜测成功。该协调留在 Docker 后端，不向通用 Runner 或 Harness 加入 Docker 分支，不重启容器或消耗业务重试预算。
