...
可见事件作为查看证据保存，不能成为新的结果接纳规则。Codex 保留实际 thread id、消息、reasoning 摘要、工具参数/结果/文件变更及已输出的其他协议字段；受信执行组合先脱敏，再写入按 Attempt 隔离的耐久记录。宿主采集时间、协议顺序与采集完成状态同时保留。未知事件保留为未解释记录，不把它当作成功、失败或完整内部思考。

可选日志接收端失败或截断时单独报告 `LOG_CAPTURE_INCOMPLETE`，不改写已经由可信执行证据判定的 Harness 状态和 outcome。Codex 会话归档按最终 JSON 转义后的 UTF-8 字节计量，预留 envelope 空间并保持在 Runner 的 16 MiB 采集上限内；过大的单份记录省略且明确标记截断，不能使整个 envelope 成为不可读的截断 JSON。

为只读查看本次会话，可以显式开启本次隔离 CODEX_HOME 的会话保存；认证文件不能随会话归档。关闭长期记忆生成与使用，候选人间不共享会话。实际未输出的思考摘要、压缩或记忆分别标明。完整隐藏 CoT 不作为可获取承诺。完整性只表示记录流是否按声明采集完成；超限、失败、协议缺失都必须可见。

这是 #39 用户要求补齐日志、容器查看和真实验证的实现选择，2026-09-15 授权完整实现。实际固定版本容器探针与真实模型验证完成后记录支持矩阵，不能以当前官方文档代替验证。DeepSeek 真实测试按同日用户提供的专用测试 key 授权执行，沿用独立凭据通路。
