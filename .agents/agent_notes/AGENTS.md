...
4. 改选后的旧方案只有在真实用户确实考虑过且给出取舍理由时，才并入同一正文的 alternatives；依据不足时只保留已确认的改选事实并注明缺失，不补造条目或理由。不设 superseded 层；独立拒绝才进入 rejected，不复制相同理由。已有决定正在讨论修订时，现行结论继续有效，待决修改另作明确标注。
