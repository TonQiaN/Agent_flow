...
try{
 if(operation==='prepare'){await app.runtime.preparePersisted(app.compiled,'run',[{id:'a',value:1},{id:'b',value:2},{id:'c',value:3}],queue.records());process.send({event:'prepared'});}
  ...
  const result=await new NodeWorker(queue,{open:app.open},systemClock,worker,['json'],900).runOnce();process.send({event:'done',result,tasks:await queue.query(),record:await queue.records().read('run')});
 }
