...
export function application(workflowId, onInvoke = async () => { }, dispatchBinding) {
    const contract = { kind: 'json', id: 'number' };
    const executor = { ...(dispatchBinding ? { dispatchBinding: () => dispatchBinding } : {}), validate() { }, contract: () => contract, contractDefinition: () => ({ ...contract, schema: { type: 'integer' } }), check: (_id, value) => Number.isSafeInteger(value) ? [] : [{ contractId: 'number', path: '$', rule: 'integer', code: 'INVALID_INTEGER' }], executionDefinition: async () => ({ schema: 'fixture-queued-calculation/v1' }), checkRecovery: async () => { },
