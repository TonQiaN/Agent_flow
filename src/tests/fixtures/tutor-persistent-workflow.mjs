...
import { DockerBackend, FileScriptRecordReader, systemClock, SqliteRunRecordStore, SqliteEffectRecordStore, FileCredentialStore, DeepSeekApiKeyCodec, DeepSeekApiKeyRunner, DeepSeekAgentDriver } from '@agentflow/integrations';
...
const [root,operation,variant,point,image,assetsPath]=process.argv.slice(2);
...
try{
 ...
 app=await createPersistentGradingApplication(root,'run',{records:store,...(operation==='run'?{source:join(root,'source')}:{ }),
  ...
  driver:artifacts=>{const d=new DeepSeekAgentDriver(runtime,artifacts,{id:'test',credentialRef:'fixture',service:'deepseek',method:'api-key',endpoint:'official',capacity:null},{timeoutMs:20000});const invoke=d.run.bind(d);d.run=(...args)=>{calls.push(app.runtime.query('run').currentNode);return invoke(...args);};return d;},
  agents:[{component:gradingComponent('marker','agent','source-files',{completed:'candidate-files'}),prompt:variant==='correct'?'correct':variant==='source-tamper'?'source-tamper':'wrong',config:{model:'deepseek-v4-flash',reasoning:'off',search:false,subagents:false}},
   {component:gradingComponent('fixer','agent','reviewed-files',{completed:'candidate-files'}),prompt:'correct',config:{model:'deepseek-v4-flash',reasoning:'off',search:false,subagents:false}}],
