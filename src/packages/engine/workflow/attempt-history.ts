...
export function validateAttemptHistory(attempts: readonly WorkflowAttemptCheckpoint[], runId: string, steps: number) {
  ...
  for (const [index, attempt] of attempts.entries()) {
    valid(attempt && typeof attempt === 'object' && !Array.isArray(attempt)
      && Object.keys(attempt).sort().join(',') === (Object.hasOwn(attempt, 'phases') ? 'identity,interrupted,launch,node,phases,resource,resultStep' : 'identity,interrupted,launch,node,resource,resultStep')
