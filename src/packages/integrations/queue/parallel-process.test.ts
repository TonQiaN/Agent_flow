...
function child(root:string,operation:string,worker=operation,leaseMs=900,finishPauseMs=0){const p=fork(new URL('../../../tests/fixtures/parallel-worker.mjs',import.meta.url),[root,operation,worker,String(leaseMs),String(finishPauseMs)],{execArgv:[],stdio:['ignore','pipe','pipe','ipc']});let stderr='';p.stderr!.on('data',b=>stderr+=b);p.stdout!.resume();const messages:any[]=[];p.on('message',m=>messages.push(m));const timeout=setTimeout(()=>p.kill('SIGKILL'),10000),exited=once(p,'exit').finally(()=>clearTimeout(timeout));
...
test('independent real Worker processes respect the parallel node cap', {timeout:15000},async t=>{
 const root=await fixture(t);await complete(root,'once');const workers=[child(root,'hold','first',30000,1100),child(root,'hold','second',30000,1100)];t.after(async()=>{for(const c of workers){if(c.p.exitCode===null&&c.p.signalCode===null)c.p.kill('SIGKILL');await c.exited;}});
