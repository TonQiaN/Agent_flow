...
import type { SubscriptionHarnessRunner } from './subscription-runner.js';
import type { SubscriptionExecution } from './subscription-runner.js';

/** Composition bridge: provider/auth details stay outside the portable acceptance engine. */
export class SubscriptionAgentDriver<P extends CredentialIdentity> implements AgentExecutionDriver {
  readonly harness: string;
  readonly #profile: P;
  readonly #options: { inputRoot: string; timeoutMs: number };
  constructor(private readonly runtime: SubscriptionHarnessRunner<P>, private readonly artifacts: ArtifactStore,
  ...
  async run(task: HarnessTask, input: FileManifest, cancellation: Cancellation): Promise<AgentExecutionHandle> {
    ...
    let execution: SubscriptionExecution;
    try {
    ...
    return new SubscriptionAgentHandle(execution, root);
  }
}

class SubscriptionAgentHandle implements AgentExecutionHandle {
  constructor(private readonly execution: SubscriptionExecution, private readonly inputRoot: string) {}
