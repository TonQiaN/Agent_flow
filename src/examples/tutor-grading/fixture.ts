...
export async function createGradingFixture(root: string, options: GradingOptions = {}) {
  ...
  for (const mode of ['correct', 'wrong', 'malformed', 'source-tamper', 'bad-evidence', 'stuck'] as const) agents.push({
    ...
    prompt: 'Grade the submitted answers against the supplied paper and answer key. Put candidate.json and the source bundle in outputs.',
  });
  for (const mode of ['correct', 'stuck'] as const) agents.push({
    ...
    prompt: 'Read gate-report.json and repair the grading candidate. Put the corrected bundle in outputs.',
  });
