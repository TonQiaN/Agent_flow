...
import type { GradingHarness } from './selected-harness.js';
import { createGradingApplication, gradingComponent, gradingWorkflow } from './flow.js';
import { digest, sourcePaths } from './gate.js';

export async function runGrading(harness: GradingHarness, preflight = false): Promise<void> {
const selected = selectGradingHarness(harness, process.env);
const { config } = selected;
if (preflight) { process.stdout.write(JSON.stringify(selected.preflight) + '\n'); return; }
const parent = selected.acceptanceRoot; await mkdir(parent, { recursive: true, mode: 0o700 });
...
const definition = gradingWorkflow({ id: 'real-grading', marker: 'matrix-marker', fixer: 'matrix-fixer', repairs: 1 });
const agents = [
  { component: gradingComponent('matrix-marker', 'agent', 'source-files', { completed: 'candidate-files' }), prompt: instructions, config },
  { component: gradingComponent('matrix-fault-marker', 'agent', 'source-files', { completed: 'candidate-files' }),
  ...
  { component: gradingComponent('matrix-fixer', 'agent', 'reviewed-files', { completed: 'candidate-files' }),
...
class RecordingDriver implements AgentExecutionDriver {
  ...
  constructor(artifacts: ArtifactStore) { this.#inner = selected.driver(artifacts, root); }
    ...
    return { get facts() { return handle.facts; }, retryCleanup: () => handle.retryCleanup(), release: async () => {
      if (!recorded) {
        ...
        if (facts.runner.capture) {
          ...
          const rawFiles: { id: string; name: string; bytes: number }[] = [];
          for (const [id, file] of Object.entries(facts.runner.capture.files)) {
            const name = `record-${rawFiles.length}.bin`;
            if (file.complete && !file.truncated && !file.error) { await cp(file.path, join(directory, name)); rawFiles.push({ id, name, bytes: file.bytes }); }
          }
          await writeFile(join(directory, 'raw-records.json'), JSON.stringify(rawFiles), { mode: 0o600 });
...
for (const fault of [false, true]) {
  const id = fault ? 'real-repair' : 'real-direct', plan = gradingWorkflow({ id, marker: fault ? 'matrix-fault-marker' : 'matrix-marker', fixer: 'matrix-fixer', repairs: 1 });
...
const summary = { root, passed, fixtureMaterial: true, modelTransport: 'configured-harness', harness, configuration: selected.preflight, originalUnchanged, emptyResources, serviceWrites: app.service.writes, runs: summaries, executions };
