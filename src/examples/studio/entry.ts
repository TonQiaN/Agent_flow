...
import {
  createGradingApplication,
  gradingComponent,
  gradingWorkflow,
} from '../tutor-grading/flow.js';
import { createPersistentGradingApplication } from '../tutor-grading/persistent.js';
import {
  selectGradingHarness,
  gradingHarness,
} from '../tutor-grading/selected-harness.js';
...
import {
  ...
  SqliteEffectRecordStore,
  SimulatedEffectService,
  DockerBackend,
...
if (operation === 'catalogue') {
  console.log(JSON.stringify(await catalogue()));
  ...
  try {
    ...
    if (key === 'recruitment')
      ...
      ['repair-example', 'parallel-map', 'parallel-fork'].includes(key)
    ) {
      const script =
          key === 'repair-example' ? 'workflow.mjs' : 'json-parallel.mjs',
        args =
          key === 'repair-example' ? ['2'] : [key.slice('parallel-'.length)];
      await new Promise<void>((resolve, reject) => {
        const child = spawn(
          process.execPath,
          ['--import', 'tsx', 'src/examples/' + script, ...args],
          { env: process.env, stdio: 'inherit' },
        );
        child.on('error', reject);
        child.on('exit', (code) =>
          code === 0 ? resolve() : reject(new Error('EXAMPLE_FAILED')),
        );
      });
    } else if (key === 'grading-fixture') {
      const app = await createGradingFixture(join(root, 'work'));
      const run = await app.run(runId);
      try {
        if (run.snapshot.outcome !== 'published')
          throw new Error('GRADING_FAILED');
      } finally {
      ...
      try {
        const selected = selectGradingHarness(
          gradingHarness(process.env['AGENTFLOW_STUDIO_HARNESS'] ?? 'deepseek'),
          { ...process.env, AGENTFLOW_ACCEPTANCE_ROOT: root },
          {
            timeoutMs: 600000,
            maxInputBytes: 144 * 1024 ** 2,
            persistSession: true,
            events: async (event) => {
        ...
        if (key === 'grading-real' || key === 'grading-persistent') {
          ...
          if (key === 'grading-real') {
            const app = await createGradingApplication(work, {
              source: join(source, 'source'),
              driver: (store) => selected.driver(store, work),
              agents,
            ...
            const journal = await SqliteEffectRecordStore.open(
              join(work, 'effects'),
            );
            try {
              const service = new SimulatedEffectService('studio-local'),
                app = await createPersistentGradingApplication(work, runId, {
                  records,
                  source,
                  driver: (store) => selected.driver(store, work),
                  agents,
          ...
          const tutorWorkspace = process.env['TUTOR_WORKSPACE'],
            python = process.env['TUTOR_PYTHON'];
          if (!tutorWorkspace || !python)
            throw new Error('TUTOR_NOT_CONFIGURED');
          const prompts = tutorMarkingPrompts({ syntheticMaterial: false });
          if (key === 'tutor-marking') {
            const app = await createTutorMarkingApplication(work, {
              source,
              tutorWorkspace,
              python,
              marker: {
                id: 'marker',
                prompt: prompts.markerPrompt,
                config: selected.config,
              },
              reviewer: {
                id: 'reviewer',
                prompt: prompts.reviewerPrompt,
                config: selected.config,
              },
              repair: {
                agent: {
            ...
            const app = await createTutorReportApplication(work, {
              source,
              tutorWorkspace,
              python,
              context: manifest.input.context,
              ...(manifest.input.completeness
                ? { submissionCompleteness: manifest.input.completeness }
                : {}),
              reporter: {
                id: 'model',
                prompt: prompts.reporterPrompt,
                config: selected.config,
              },
              driver: (store) => selected.driver(store, work),
            });
    ...
    await writeFile(
      ...
      JSON.stringify({ finishedAt: Date.now(), error: null }),
      { mode: 0o600 },
    );
  } catch (error) {
    await writeFile(
