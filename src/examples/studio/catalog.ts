...
export interface CatalogueItem { id: string; title: string; description: string; entrypoint: string; input: string; requires: string[]; definition: unknown }
export async function catalogue(): Promise<CatalogueItem[]> {
  ...
  const root = await mkdtemp(join(tmpdir(), 'af-catalogue-')), before = process.env['AGENTFLOW_HISTORY_DISABLED']; process.env['AGENTFLOW_HISTORY_DISABLED'] = '1';
  try {
    const dummy = { id: 'model', prompt: 'Descriptor only; never executed.', config: { mode: 'correct' } }, setup = { source: join(root, 'source'), tutorWorkspace: process.env['TUTOR_WORKSPACE'] ?? '/unconfigured/tutor', python: process.env['TUTOR_PYTHON'] ?? '/usr/bin/python3', driver: (store: any) => new GradingFixtureDriver(store, join(root, 'unused')) };
    const marking = await createTutorMarkingApplication(join(root, 'marking'), { ...setup, marker: { ...dummy, id: 'marker' }, reviewer: { ...dummy, id: 'reviewer' }, repair: { agent: { ...dummy, id: 'reviewer-repair' }, maxRounds: 2 } });
    const report = await createTutorReportApplication(join(root, 'report'), { ...setup, context: fixtureContext, reporter: dummy });
    base.push({ id: 'tutor-marking', title: '扫描试卷批改', description: '现有 Tutor 工作流：接收材料、批改、独立复核和返工。需安装 Tutor。', entrypoint: 'tutor-marking/application.ts', input: 'tutor-marking', requires: ['docker', 'harness', 'tutor'], definition: marking.compiled.definition },
      { id: 'tutor-report', title: 'Tutor 学习报告', description: '现有报告流程：读取已批改材料、生成报告、验证并导出 PDF。', entrypoint: 'tutor-report/application.ts', input: 'tutor-report', requires: ['docker', 'harness', 'tutor'], definition: report.compiled.definition });
    return base;
  } finally { if (before === undefined) delete process.env['AGENTFLOW_HISTORY_DISABLED']; else process.env['AGENTFLOW_HISTORY_DISABLED'] = before; await rm(root, { recursive: true, force: true }); }
}
