...
export function toolIsolateArguments(temporary, mode, argv, cwd = '/task/work') {
    if (!/^\/tmp\/agentflow-tools-[A-Za-z0-9]+$/.test(temporary) || !['read-only', 'workspace-write'].includes(mode) || !argv.length || typeof cwd !== 'string' || !cwd.startsWith('/') || cwd.includes('\0'))
